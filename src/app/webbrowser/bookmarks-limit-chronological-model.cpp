...
/*!
    \class BookmarksLimitChronologicalModel
    \brief Identity model that limits the number of rows returned by a bookmarks
    model in chronological order

    BookmarksLimitChronologicalModel is a proxy model that limits the number
    of rows returned by a BookmarksChronologicalModel
    (i.e. only the first N bookmarks are returned).
*/
BookmarksLimitChronologicalModel::BookmarksLimitChronologicalModel(QObject* parent)
...
BookmarksChronologicalModel* BookmarksLimitChronologicalModel::sourceModel() const
{
    return qobject_cast<BookmarksChronologicalModel*>(QIdentityProxyModel::sourceModel());
}

void BookmarksLimitChronologicalModel::setSourceModel(BookmarksChronologicalModel* sourceModel)
{
...
int BookmarksLimitChronologicalModel::rowCount(const QModelIndex &parent) const
{
...
int BookmarksLimitChronologicalModel::limit() const
{
    return m_limit;
}

void BookmarksLimitChronologicalModel::setLimit(int limit)
{
    if (limit != m_limit) {
...
void BookmarksLimitChronologicalModel::sourceRowsAboutToBeInserted(const QModelIndex &parent, int start, int end)
{
    if (m_limit < 0) {
...
void BookmarksLimitChronologicalModel::sourceRowsAboutToBeRemoved(const QModelIndex &parent, int start, int end)
{
    if (m_limit < 0) {
...
void BookmarksLimitChronologicalModel::sourceRowsInserted(const QModelIndex & /*parent*/, int /*start*/, int /*end*/)
{
...
void BookmarksLimitChronologicalModel::sourceRowsRemoved(const QModelIndex & /*parent*/, int /*start*/, int /*end*/)
{
    if (m_sourceRemoving) {
        endRemoveRows();
        m_sourceRemoving = false;
